...
<xsl:stylesheet
    ...
    <xsl:template name="itemSummaryView-DIM-fields">
      ...
      <xsl:choose>
		...
		<!-- Type row -->
		 <xsl:when test="$clause = 13 and (dim:field[@element='type' and not(@qualifier)])">
                    <tr class="ds-table-row {$phase}">
		...
		<!-- Language row -->
		<xsl:when test="$clause = 14 and (dim:field[@element='language' and @qualifier='iso'])">
                    <tr class="ds-table-row {$phase}">
		...
		<!-- Status row -->
		<xsl:when test="$clause = 15 and (dim:field[@element='identifier' and @qualifier='status'])">
                    <tr class="ds-table-row {$phase}">
		...
		<xsl:when test="$clause = 16 and (dim:field[@element='cplace' and @qualifier='country'])">
                    <tr class="ds-table-row {$phase}">
		...
		<!-- Region Focus row --> 
		<xsl:when test="$clause = 17 and (dim:field[@element='rplace' and @qualifier='region'])">
                    <tr class="ds-table-row {$phase}">
		  ...
		  <!-- ILRI Subject row --> 
		<xsl:when test="$clause = 18 and (dim:field[@element='isubject' and @qualifier='ilrisubject'])">
                    <tr class="ds-table-row {$phase}">
		...
		<!-- Google URL row -->
		<xsl:when test="$clause = 20 and (dim:field[@element='identifier' and @qualifier='googleurl'])">
                    <tr class="ds-table-row {$phase}">
		...
		<!-- Fund row --> 
		<xsl:when test="$clause = 21 and (dim:field[@element='identifier' and @qualifier='fund'])">
                    <tr class="ds-table-row {$phase}">
