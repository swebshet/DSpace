<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="dri:div[@id='aspect.statistics.GraphEditor.div.javascriptvars' or @id='aspect.statistics.ContentAnalysisTransformer.div.javascriptvars']" priority="3">
    ...
    <xsl:template match="dri:div[ancestor::dri:div[@id='aspect.statistics.GraphEditor.div.wrapper'] or ancestor::div[@id='aspect.statistics.ContentAnalysisTransformer.div.wrapper']]" priority="1">
    ...
    <xsl:template match="dri:div[@interactive='yes' and (ancestor::dri:div[@id='aspect.statistics.GraphEditor.div.wrapper'] or ancestor::div[@id='aspect.statistics.OldGraphEditor.div.wrapper']or ancestor::div[@id='aspect.statistics.ContentAnalysisTransformer.div.wrapper'])]" priority="2">
        <xsl:apply-templates select="dri:head"/>
        <xsl:apply-templates select="@pagination">
            <xsl:with-param name="position">top</xsl:with-param>
        </xsl:apply-templates>
    ...
    <xsl:template match="dri:div[@id='aspect.statistics.editorparts.GraphSettingsTransformer.div.graph'] | dri:div[@id='aspect.statistics.editorparts.GraphTransformer.div.graph'] | dri:div[@id='aspect.statistics.ContentAnalysisTransformer.div.graph']" priority="3">
        <xsl:apply-templates select="dri:head"/>
        <div>
            <xsl:call-template name="standardAttributes"/>
            <xsl:for-each select="dri:p[not(@id='aspect.statistics.editorparts.GraphTransformer.p.csv')]">
                <img>
                    <xsl:attribute name="src">
                        <xsl:value-of select="dri:figure/@source"/>-img<xsl:value-of select="dri:figure/@target"/>
                    </xsl:attribute>
                    <xsl:attribute name="usemap">
                        <xsl:text>#</xsl:text>
                    ...
                    <xsl:attribute name="class">
                        <xsl:value-of select="dri:figure/@rend"/>
                    </xsl:attribute>
                </img>
                <xsl:copy-of select="map"/>
            </xsl:for-each>
            <xsl:apply-templates select="dri:p[@id='aspect.statistics.editorparts.GraphTransformer.p.csv']"/>
        </div>
    </xsl:template>
    ...
    <xsl:template match="dri:p[@id='aspect.statistics.ContentAnalysisTransformer.p.csv']">
        <xsl:element name="p">
            <xsl:call-template name="standardAttributes"/>
            <!--<xsl:apply-templates select="self::node()[not(@id = 'statistics.ContentAnalysisTransformer.p.csv')]" />-->
            ...
            <xsl:apply-templates select="dri:field[@id != 'aspect.statistics.ContentAnalysisTransformer.field.csv']"/>
            <!--<xsl:apply-templates select="*[@id != 'statistics.ContentAnalysisTransformer.field.csv']"/>-->
            <xsl:choose>
                <xsl:when test="dri:field[@id='aspect.statistics.ContentAnalysisTransformer.field.csv']">
                    <xsl:element name="input">
                        <xsl:attribute name="type">image</xsl:attribute>
            ...
            <a>
                <xsl:attribute name="id">aspect_statistics_ContentAnalysisTransformer_field_csv_a</xsl:attribute>
    ...
    <xsl:template match="dri:p[@n='csv']">
        <xsl:element name="p">
            <xsl:call-template name="standardAttributes"/>
            ...
            <!--<xsl:for-each select="./dri:field[@id!='aspect.statistics.editorparts.DataTableTransformer.field.csv' and (@rend!='manualrender' or not(@rend))]">-->
            <!--<xsl:copy-of select="."/>-->
            <!--</xsl:for-each>-->
            <!--<xsl:for-each select="./dri:field">-->
            <!--<xsl:copy-of select="."/>-->
            <!--</xsl:for-each>-->
            <xsl:apply-templates select="dri:field[@n!='csv'   and (@rend!='manualrender' or not(@rend))]"/>
            ...
            <button class="ui-corner-all gen_button">
            ...
            <button class="ui-corner-all gen_button">
                <xsl:choose>
                    <xsl:when test="contains(@id,'DataTableTransformer')"><xsl:attribute name="id">aspect_statistics_editorparts_DataTableTransformer_field_subscribe_a</xsl:attribute></xsl:when>
                    <xsl:otherwise><xsl:attribute name="id">aspect_statistics_editorparts_GraphTransformer_field_subscribe_a</xsl:attribute></xsl:otherwise>
                </xsl:choose>

                <xsl:attribute name="style">cursor:pointer;</xsl:attribute>
                <xsl:attribute name="data-action">
                    <xsl:value-of select="concat($context-path,'/atmire/reporting-suite/subscribe')"/>
                </xsl:attribute>
                <i18n:text>xmlui.reporting-suite.statistics.graph-editor.export.subscribe</i18n:text>
            </button>


            <span class="subscriptionsuccess notice success"> <i18n:text>xmlui.reporting-suite.statistics.graph-editor.export.subscribe.success</i18n:text></span>
            <span class="subscriptionprocessing notice"> <i18n:text>xmlui.reporting-suite.statistics.graph-editor.export.subscribe.processing</i18n:text></span>
            <span class="subscriptionerror notice failure"> <i18n:text>xmlui.reporting-suite.statistics.graph-editor.export.subscribe.error</i18n:text><a href="#" onclick="window.location.reload(true);"><i18n:text>xmlui.reporting-suite.statistics.graph-editor.export.subscribe.reload</i18n:text></a></span>
            <xsl:if test="contains(@id,'DataTableTransformer')">
                <div class="subscribedialog">
                    <label>
                        <i18n:text>xmlui.reporting-suite.statistics.graph-editor.export.subscribe.period</i18n:text>
                        <xsl:apply-templates
                                select="dri:field[@n='period']"/>


                    </label>

                    <label class="reporttitle">
                        <i18n:text>xmlui.reporting-suite.statistics.graph-editor.export.subscribe.name</i18n:text>
                        <br />
                    ...
                    <label class="reporttitle">
                        <i18n:text>xmlui.reporting-suite.statistics.graph-editor.export.subscribe.emails</i18n:text>
                        <br />
                        <input type="email" name="emails" required="required">
                            <xsl:attribute name="value">
                                <xsl:value-of select="dri:field[@n='emails']/dri:value/text()"/>
                            </xsl:attribute>
                        </input>
                        <img id="addButton"><xsl:attribute name="src"><xsl:value-of select="concat($context-path,'/aspects/Datatables/shared/images/details_open.png')"/></xsl:attribute></img>
                    </label>
    ...
    <!--<xsl:template match="dri:list[@id='aspect.statistics.ContentAnalysisTransformer.list.dataset_items_1_scope' or @id='aspect.statistics.ContentAnalysisTransformer.list.dataset_items_2_scope']">-->
    <!--<li>-->
    <!--<ul>-->
    ...
    <xsl:template match="dri:p[@id='aspect.statistics.StatletTransformer.p.statslink']">
        <p>
            ...
            <xsl:apply-templates select="*[not(name()='xref')]"/>
        </p>
    </xsl:template>
    ...
    <xsl:template match="dri:p[dri:field/@id = 'aspect.statistics.editorparts.TimeFilterTransformer.field.update']">
        <div name="{dri:field/@n}" class="time-update-button ui-corner-all gen_button" onmouseover="$(this).addClass('button-hover');" onmouseout="$(this).removeClass('button-hover');" onclick="$(document).cua('updateData');return false;">
            <xsl:choose>
    ...
    <xsl:template match="dri:p[dri:field/@id = 'aspect.statistics.editorparts.UpdateButtonTransformer.field.update']">
        <div name="{dri:field/@n}" class="update-button" onclick="$(document).cua('updateData',false,true);return false;">
            <xsl:choose>
                <xsl:when test="dri:field/dri:value/i18n:text">
                    <i18n:text><xsl:value-of select="dri:field/dri:value"/></i18n:text>
                </xsl:when>
    ...
    <xsl:template match="dri:cell[starts-with(dri:field/@id,'aspect.statistics.editorparts.DataTableTransformer.field.swapRowsAndCols')]">
        <td>
            <xsl:call-template name="standardAttributes">
            ...
            <input name="{dri:field/@n}" type="button" swapAction="{$swapAction}">
                <xsl:attribute name="value">
                    <xsl:value-of select="dri:field/dri:value"/>
                </xsl:attribute>
                <xsl:if test="dri:field/dri:value/i18n:text">
    ...
    <xsl:template match="dri:div[@id='aspect.statistics.editorparts.TimeFilterTransformer.div.timefilter' or @id='aspect.statistics.SearchStatistics.div.timefilter']//dri:xref[@target='showOptions']" priority="1">
    ...
    <xsl:template match="dri:div[@id='aspect.statistics.editorparts.TimeFilterTransformer.div.timefilter' or @id='aspect.statistics.SearchStatistics.div.timefilter']//dri:xref[@target='hideOptions']" priority="1">
    ...
    <xsl:template match="dri:xref[@target = 'checkAllHierarchical']">
        <a onclick="checkAllHierarchical(this); return false;">
            <xsl:attribute name="href"><xsl:value-of select="@target"/></xsl:attribute>
            <xsl:attribute name="class"><xsl:value-of select="@rend"/></xsl:attribute>
            <xsl:apply-templates />
        </a>
    ...
    <xsl:template match="dri:xref[@target = 'checkNoneHierarchical']">
        <a onclick="checkNoneHierarchical(this); return false;">
            <xsl:attribute name="href"><xsl:value-of select="@target"/></xsl:attribute>
            <xsl:attribute name="class"><xsl:value-of select="@rend"/></xsl:attribute>
            <xsl:apply-templates />
        </a>
    ...
    <xsl:template match="dri:field[ancestor::dri:div[@id='aspect.statistics.GraphEditor.div.wrapper'] or ancestor::div[@id='aspect.statistics.OldGraphEditor.div.wrapper']or ancestor::div[@id='aspect.statistics.ContentAnalysisTransformer.div.wrapper']]" mode="compositeComponent">
        <xsl:choose>
            <xsl:when test="@type = 'checkbox'  or @type='radio'">
                <xsl:apply-templates select="." mode="normalField"/>
                <br/>
    ...
    <xsl:template match="dri:p[dri:field/@id='aspect.statistics.ContentAnalysisTransformer.field.add_category']">
        <div name="{dri:field/@n}" class="add-category-button atmire-button-dark ui-corner-all clearfix" onmouseover="$(this).addClass('button-hover');" onmouseout="$(this).removeClass('button-hover');" >
            <img style="float: left;">
                <xsl:attribute name="src">
                    <xsl:value-of select="$context-path"/>
                    <xsl:text>/aspects/Datatables/shared/images/details_open.png</xsl:text>
                </xsl:attribute>
            </img>
    ...
    <xsl:template match="dri:div[@id='aspect.statistics.ContentAnalysisTransformer.div.remove']">
        <div>
            <xsl:call-template name="standardAttributes"/>
            <p>
                <xsl:apply-templates select="dri:p/dri:figure"/>
                <span class="vAlign">
                    <i18n:text><xsl:value-of select="dri:p"/></i18n:text>
                </span>
            </p>
        </div>
    </xsl:template>

    <xsl:template match="dri:field[ancestor::dri:field[contains(@n,'Time')]]" mode="compositeComponent">
        <xsl:choose>
            <xsl:when test="@type = 'checkbox'  or @type='radio'">
                <xsl:apply-templates select="." mode="normalField"/>
                <xsl:if test="dri:label">
    ...
    <xsl:template match="dri:div[@id='aspect.artifactbrowser.GeoStats.div.geo-stats' or @id='aspect.statistics.MostPopularItems.div.geo-stats' ]">
        <script type="text/javascript">
            var atmire;
            if(atmire === undefined){
            atmire={};
            }
            atmire.contextPath = '<xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]"/>';
        </script>
    ...
    <xsl:template match="dri:table[@id='aspect.statistics.SearchStatistics.table.searchstattable']/dri:row[@role='header']/dri:cell">
        <th>
            <xsl:call-template name="standardAttributes">
                <xsl:with-param name="class">ds-table-header-cell</xsl:with-param>
            </xsl:call-template>
            <nobr>
