...
public class ItemAdapter extends AbstractAdapter
{
	...
	private static List<ItemAdapterMetadataPlugin> metadataPlugins = new DSpace().getServiceManager().getServicesByType(ItemAdapterMetadataPlugin.class);
	 ...
	 */
	public ItemAdapter(Request request, Context context, Item item, String contextPath)
	{
		super(contextPath);
		this.item = item;
	...
	protected void renderDescriptiveSection() throws WingException, SAXException, CrosswalkException, IOException, SQLException
	{
		...
		if(dmdTypes.size() == 0 || dmdTypes.contains("DIM"))
		{
			...
			LinkedList<Metadatum> dcvs = new LinkedList<Metadatum>();
			dcvs.addAll(Arrays.asList(item.getMetadata(Item.ANY, Item.ANY, Item.ANY, Item.ANY)));
			for (ItemAdapterMetadataPlugin plugin : metadataPlugins) {
				try {
					dcvs.addAll(plugin.generateMetadataForItem(request, context, item));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	protected void renderFileSection() throws SQLException, SAXException
	{
		...
		for (Bundle bundle : bundles)
		{
			...
			for (Bitstream bitstream : bundle.getBitstreams())
			{
				...
				renderFileWithAllowed(context, item, bitstream, fileID, groupID, admIDs);
	...
	protected void renderFileWithAllowed(Context context, Item item, Bitstream bitstream, String fileID, String groupID, String admID) throws SAXException, SQLException {
		AttributeMap attributes;
