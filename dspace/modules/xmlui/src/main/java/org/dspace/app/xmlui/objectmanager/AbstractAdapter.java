...
public abstract class AbstractAdapter
{
	 ...
	 */
	protected final void renderFile(Item item, Bitstream bitstream, String fileID, String groupID) throws SAXException, SQLException
	{
	...
	protected void embargoCheck(Context context, Bitstream bitstream, AttributeMap attributes) throws SQLException {
		//Check if our current user can read the file if so the link should still be shown
		boolean hasAccess;
		if (context != null) {
			hasAccess = AuthorizeManager.authorizeActionBoolean(context, bitstream, Constants.READ);
		} else {
			hasAccess = true;
		}

		attributes.put("READRIGHTS", hasAccess);
		//Check if we have an embargo
		Metadatum[] embargoDates = bitstream.getMetadata("atmire", "embargo", "date", Item.ANY);
		if(0 < embargoDates.length){
			//We have an embargo
			//First check if our embargo is still active
			boolean isActive = true;
			Metadatum[] embargoConfirms = bitstream.getMetadata("atmire", "embargo", "confirmed", Item.ANY);
			if(0 < embargoConfirms.length)
				isActive = Boolean.valueOf(embargoConfirms[0].value);

			if(isActive){
				//our embargo is active
				attributes.put("PERMISSION", "embargo");
				//Also put down the embargo date
				SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
				attributes.put("EMBARGODATE", dateFormat.format(new DCDate(embargoDates[0].value).toDate()));
			}else{
				//We have a private file
				attributes.put("PERMISSION", "private");
			}
		}else{
			//Check if our public permissions have been confirmed
			Metadatum[] embargoConfirms = bitstream.getMetadata("atmire", "embargo", "confirmed", Item.ANY);
			boolean isConfirmed = true;
			if(0 < embargoConfirms.length)
				isConfirmed = Boolean.valueOf(embargoConfirms[0].value);

			if(isConfirmed)
				attributes.put("PERMISSION", "public");
			else
				attributes.put("PERMISSION", "private");
		}
	}
