...
function doManageGroups()
{
    ...
    do {
        ...
        else if ((cocoon.request.get("submit_save_cua") || cocoon.request.get("submit_hide_cua")) && cocoon.request.get("select_group"))
        {
            // Delete a set of groups
            var groupIDs = cocoon.request.getParameterValues("select_group");
            result = FlowGroupUtils.processEnableDisableCUAGroups(getDSContext(),groupIDs, cocoon.request.get("submit_save_cua") != null);
            highlightID = -1;
        }
...
function doEditGroup(groupID)
{
    ...
    do {
        ...
        if (cocoon.request.get("submit_cancel"))
        {
       	...
       	{
       		result = FlowGroupUtils.processSaveGroup(getDSContext(),groupID,escape(groupName),memberEPeopleIDs,memberGroupIDs, cocoon.request);
