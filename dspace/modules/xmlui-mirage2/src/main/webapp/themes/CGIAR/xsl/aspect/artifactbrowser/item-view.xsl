...
<xsl:stylesheet
                ...
                <div>
                    <xsl:for-each select="dim:field[@element='description' and @qualifier='sponsorship']">
                        <xsl:choose>
                            <xsl:when test="node()">
                                <a target="_black">
                                    <!-- TODO find out what discover filter is required here
                                    <xsl:attribute name="href">
                                        <xsl:value-of select="concat($context-path,'/discover?filtertype=crpsubject&amp;filter_relational_operator=equals&amp;filter=',url:encode(node()))"></xsl:value-of>
                                    </xsl:attribute>
                                       -->
    ...
    <xsl:template name="itemSummaryView-DIM-related-material">
        <xsl:if test="dim:field[@mdschema='cg' and @element='link' ]">
            <div class="simple-item-view-description item-page-field-wrapper table">
                <h5 class="bold">
                    <i18n:text>xmlui.dri2xhtml.METS-1.0.item-related-material</i18n:text>
                </h5>
                <div>
                    <xsl:for-each select="dim:field[@mdschema='cg' and @element='link']">
                        <xsl:choose>
                            <xsl:when test="node()">
                        ...
                        <xsl:if test="count(following-sibling::dim:field[@mdschema='cg' and @element='link']) != 0">
                            <xsl:text>; </xsl:text>
                        </xsl:if>
                    </xsl:for-each>
                    <xsl:if test="count(dim:field[@mdschema='cg' and @element='link'] ) &gt; 1">
                        <div class="spacer">&#160;</div>
                    </xsl:if>
                </div>
            </div>
